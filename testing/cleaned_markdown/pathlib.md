...
`os.PathLike` interface where the
`~os.PathLike.__fspath__` method returns
...
Pure path objects implement the `os.PathLike` interface, allowing them to be used anywhere the interface
is accepted.
...
A subclass of `PurePath`, this path
...
Paths are immutable and `hashable`. Paths
...
Similarly, calling `bytes` on a path
...
Calling `bytes` is only recommended
...
The implementation of the `os.path` module
...
`slices <slice>` and negative index
values.
:::
::::
...
This method is similar to `~PurePath.full_match`, but empty patterns aren\'t allowed
(`ValueError` is raised), the recursive
...
*other*. If it\'s impossible, `ValueError`
is raised:
...
This function is part of `PurePath` and
...
symlinks are present in the path; call `~Path.resolve` first if necessary to resolve symlinks.
::::
...
Return a new path with the `name`
...
Return a new path with the `stem`
changed. If the original path doesn\'t have a name, ValueError is
raised:
...
versions, `ValueError` is raised if a
...
created, such as from `parent` and
`relative_to`. Subclasses may override
...
A subclass of `Path` and
`PurePosixPath`, this class represents
...
::: versionchanged
3.13 Raises `UnsupportedOperation` on
Windows. In previous versions, `NotImplementedError` was raised instead.
:::
::::

:::: WindowsPath(*pathsegments)
A subclass of `Path` and
`PureWindowsPath`, this class represents
...
Some concrete path methods can raise an `OSError` if a system call fails (for example because the path
doesn\'t exist).
...
`ValueError` is raised if the URI does not
...
Calling this method from `PurePath`
rather than `Path` is possible but
deprecated. The method\'s use of `os.fsencode` makes it strictly impure.
:::
::::
...
`OSError` is raised in strict mode, and no
exception is raised in non-strict mode. In previous versions,
`RuntimeError` is raised no matter the
value of *strict*.
:::
:::::
...
::: versionchanged
3.13 Raises `UnsupportedOperation` if
`os.readlink` is not available. In
previous versions, `NotImplementedError`
was raised.
:::
:::::
...
`~Path.is_char_device`,
`~Path.is_fifo`,
`~Path.is_socket` now return `False`
...
`OSError` exception from the operating
system. In previous versions, some kinds of `OSError` exception are raised, and others suppressed. The new
behaviour is consistent with `os.path.exists`, `os.path.isdir`, etc. Use
`~Path.stat` to retrieve the file status
...
containing information about this path, like `os.stat`. The result is looked up at each call to this method.
...
Like `Path.stat` but, if the path points
...
missing. Use `Path.stat` to distinguish
between these cases.
...
`Path.stat` to distinguish between these
cases.
...
symbolic link. Use `Path.stat` to
...
Return `True` if the path is a `mount point`: a point in a file system where a different file system has
been mounted. On POSIX, the function checks whether *path*\'s parent,
`path/..`, is on a different device than
*path*, or whether `path/..` and *path*
...
something other than a FIFO. Use `Path.stat` to distinguish between these cases.
:::
...
`os.path.samefile` and
`os.path.samestat`.

An `OSError` can be raised if either file
...
A `~pathlib.types.PathInfo` object that
...
::: versionchanged
3.13 Any `OSError` exceptions raised from
...
::: seealso
`pathlib-pattern-language` and
`Path.glob` documentation.
:::
...
*dirpath* is a `Path` to the directory
...
By default, `Path.walk` does not follow
...
`Path.walk` does not keep track of the
...
`Path.walk` assumes the directories it
...
is false, `Path.walk` will still try to
...
Unlike `os.walk`,
`Path.walk` lists symlinks to directories
...
bottom-up is essential as `rmdir`
...
::: versionchanged
3.13 Raises `UnsupportedOperation` if
`os.symlink` is not available. In
previous versions, `NotImplementedError`
was raised.
:::
::::::
...
::: versionchanged
3.13 Raises `UnsupportedOperation` if
`os.link` is not available. In previous
versions, `NotImplementedError` was
raised.
:::
:::::::
...
to `Path.copy`. Returns a new
`!Path` instance pointing to the copy.
...
`FileExistsError` will be raised. *target*
...
It is implemented in terms of `os.rename`
...
non-empty directory, then `OSError` is
raised.

If both paths are on the same filesystem, the move is performed with
`os.replace`. Otherwise, this path is
...
should be an existing directory. Returns a new `!Path` instance pointing to the moved path.
...
`FileNotFoundError` is raised if the path
does not exist.

If *missing_ok* is true, `FileNotFoundError` exceptions will be ignored (same behavior as the POSIX
`rm -f` command).
...
`KeyError` is raised if the file\'s user
...
::: versionchanged
3.13 Raises `UnsupportedOperation` if the
`pwd` module is not available. In earlier
versions, `NotImplementedError` was
raised.
:::
...
`KeyError` is raised if the file\'s group
...
::: versionchanged
3.13 Raises `UnsupportedOperation` if the
`grp` module is not available. In earlier
versions, `NotImplementedError` was
raised.
:::
...
Like `Path.chmod` but, if the path points
...
`Path.glob` and
`Path.rglob` differ slightly from those
by the `glob` module:
...
3.  \"`**`\" pattern components do not follow symlinks by default in
    ...
    `glob.glob`, but you can pass
    `recurse_symlinks=True` to `Path.glob` for compatible behaviour.
4.  Like all `PurePath` and
    `Path` objects, the values returned
    from `Path.glob` and
    `Path.rglob` don\'t include trailing
    slashes.
...
functions to their corresponding `PurePath`/`Path` equivalent.
...
The `!pathlib.types` module provides types
