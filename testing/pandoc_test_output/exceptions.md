...
`malloc`{.interpreted-text role="c:func"} function), the interpreter may
...
A numeric error code from the C variable `errno`{.interpreted-text
role="c:data"}.
:::
...
is formatted by the C functions `!perror`{.interpreted-text
role="c:func"} under POSIX, and `!FormatMessage`{.interpreted-text
role="c:func"} under Windows.
:::
...
- `Joining <threading.Thread.join>`{.interpreted-text role="meth"} a
  running daemon thread.
- `os.fork`{.interpreted-text role="func"},
- acquiring a lock such as `threading.Lock`{.interpreted-text
  role="class"}, when it is known that the operation would otherwise
  deadlock.
...
`threading.Thread.join`{.interpreted-text role="meth"} can now raise
this exception.
:::
...
`threading.Lock`{.interpreted-text role="meth"} or
`threading.RLock`{.interpreted-text role="meth"}.
:::
::::::
...
to C\'s `!exit`{.interpreted-text role="c:func"} function); if it is
...
non-blocking operation. Corresponds to `errno`{.interpreted-text
role="c:data"} `~errno.EAGAIN`{.interpreted-text role="py:const"},
`~errno.EALREADY`{.interpreted-text role="py:const"},
`~errno.EWOULDBLOCK`{.interpreted-text role="py:const"} and
`~errno.EINPROGRESS`{.interpreted-text role="py:const"}.
...
`errno`{.interpreted-text role="c:data"}
`~errno.ECHILD`{.interpreted-text role="py:const"}.
:::
...
Corresponds to `errno`{.interpreted-text role="c:data"}
`~errno.EPIPE`{.interpreted-text role="py:const"} and
`~errno.ESHUTDOWN`{.interpreted-text role="py:const"}.
:::
...
`errno`{.interpreted-text role="c:data"}
`~errno.ECONNABORTED`{.interpreted-text role="py:const"}.
:::
...
`errno`{.interpreted-text role="c:data"}
`~errno.ECONNREFUSED`{.interpreted-text role="py:const"}.
:::
...
`errno`{.interpreted-text role="c:data"}
`~errno.ECONNRESET`{.interpreted-text role="py:const"}.
:::
...
Corresponds to `errno`{.interpreted-text role="c:data"}
`~errno.EEXIST`{.interpreted-text role="py:const"}.
:::
...
Corresponds to `errno`{.interpreted-text role="c:data"}
`~errno.ENOENT`{.interpreted-text role="py:const"}.
:::
...
Corresponds to `errno`{.interpreted-text role="c:data"}
`~errno.EINTR`{.interpreted-text role="py:const"}.
...
`errno`{.interpreted-text role="c:data"}
`~errno.EISDIR`{.interpreted-text role="py:const"}.
:::
...
it were a directory. Corresponds to `errno`{.interpreted-text
role="c:data"} `~errno.ENOTDIR`{.interpreted-text role="py:const"}.
:::
...
`errno`{.interpreted-text role="c:data"}
`~errno.EACCES`{.interpreted-text role="py:const"},
`~errno.EPERM`{.interpreted-text role="py:const"}, and
`~errno.ENOTCAPABLE`{.interpreted-text role="py:const"}.

::: versionchanged
3.11.1 WASI\'s `~errno.ENOTCAPABLE`{.interpreted-text role="py:const"}
...
`errno`{.interpreted-text role="c:data"}
`~errno.ESRCH`{.interpreted-text role="py:const"}.
:::
...
to `errno`{.interpreted-text role="c:data"}
`~errno.ETIMEDOUT`{.interpreted-text role="py:const"}.
:::
...
Exception groups are `generic <generics>`{.interpreted-text role="ref"}
