...
adds classes with a minimum of new syntax and semantics. It is a mixture
of the class mechanisms found in C++ and Modula-3. Python classes
provide all the standard features of Object Oriented Programming: the
class inheritance mechanism allows multiple base classes, a derived
class can override any methods of its base class or classes, and a
method can call the method of a base class with the same name. Objects
can contain arbitrary amounts and kinds of data. As is true for modules,
...
are *public* (except see below `tut-private`{.interpreted-text
role="ref"}), and all member functions are *virtual*. As in Modula-3,
there are no shorthands for referencing the object\'s members from its
...
(containing functions such as `abs`{.interpreted-text role="func"}, and
built-in exception names); the global names in a module; and the local
names in a function invocation. In a sense the set of attributes of an
object also form a namespace. The important thing to know about
namespaces is that there is absolutely no relation between names in
different namespaces; for instance, two different modules may both
...
the `del`{.interpreted-text role="keyword"} statement. For example,
`del modname.the_answer` will remove the attribute
`!the_answer`{.interpreted-text role="attr"} from the object named by
`modname`.
...
a module called `__main__`{.interpreted-text role="mod"}, so they have
their own global namespace. (The built-in names actually also live in a
module; this is called `builtins`{.interpreted-text role="mod"}.)
...
`nonlocal`{.interpreted-text role="keyword"} statement can be used; if
not declared nonlocal, those variables are read-only (an attempt to
write to such a variable will simply create a *new* local variable in
the innermost scope, leaving the identically named outer variable
unchanged).

Usually, the local scope references the local names of the (textually)
current function. Outside functions, the local scope references the same
namespace as the global scope: the module\'s namespace. Class
...
A special quirk of Python is that \-- if no `global`{.interpreted-text
role="keyword"} or `nonlocal`{.interpreted-text role="keyword"}
...
scope: in particular, `import`{.interpreted-text role="keyword"}
statements and function definitions bind the module or function name in
the local scope.

The `global`{.interpreted-text role="keyword"} statement can be used to
indicate that particular variables live in the global scope and should
be rebound there; the `nonlocal`{.interpreted-text role="keyword"}
...
and namespaces, and how `global`{.interpreted-text role="keyword"} and
`nonlocal`{.interpreted-text role="keyword"} affect variable binding:
...
*scope_test*\'s binding of *spam*. The `nonlocal`{.interpreted-text
role="keyword"} assignment changed *scope_test*\'s binding of *spam*,
and the `global`{.interpreted-text role="keyword"} assignment changed
the module-level binding.

You can also see that there was no previous binding for *spam* before
the `global`{.interpreted-text role="keyword"} assignment.
...
Class definitions, like function definitions (`def`{.interpreted-text
role="keyword"} statements) must be executed before they have any
effect. (You could conceivably place a class definition in a branch of
an `if`{.interpreted-text role="keyword"} statement, or inside a
function.)
...
as the local scope \-\-- thus, all assignments to local variables go
...
definition header (`!ClassName`{.interpreted-text role="class"} in the
example).
...
`MyClass.i` by assignment. `~type.__doc__`{.interpreted-text
role="attr"} is also a valid attribute, returning the docstring
...
special method named `~object.__init__`{.interpreted-text role="meth"},
like this:

    def __init__(self):
        self.data = []

When a class defines an `~object.__init__`{.interpreted-text
role="meth"} method, class instantiation automatically invokes
`!__init__`{.interpreted-text role="meth"} for the newly created class
instance. So in this example, a new, initialized instance can be
obtained by:

    x = MyClass()

Of course, the `~object.__init__`{.interpreted-text role="meth"} method
may have arguments for greater flexibility. In that case, arguments
given to the class instantiation operator are passed on to
`!__init__`{.interpreted-text role="meth"}. For example, :
...
to. For example, if `x` is the instance of `!MyClass`{.interpreted-text
role="class"} created above, the following piece of code will print the
...
If `x = MyClass()`, as above, this will return the string
`'hello world'`. However, it is not necessary to call a method right
away: `x.f` is a method object, and can be stored away and called at a
...
definition for `!f`{.interpreted-text role="meth"} specified an
argument. What happened to the argument? Surely Python raises an
exception when a function that requires an argument is called without
...
As discussed in `tut-object`{.interpreted-text role="ref"}, shared data
can have possibly surprising effects involving
`mutable`{.interpreted-text role="term"} objects such as lists and
...
it possible to enforce data hiding \-\-- it is all based upon
convention. (On the other hand, the Python implementation, written in C,
can completely hide implementation details and control access to an
object if necessary; this can be used by extensions to Python written in
C.)
...
as name conflicts are avoided \-\-- again, a naming convention can save
...
Now `f`, `g` and `h` are all attributes of class `!C`{.interpreted-text
role="class"} that refer to function objects, and consequently they are
all methods of instances of `!C`{.interpreted-text role="class"} \-\--
`h` being exactly equivalent to `g`. Note that this practice usually
...
in the next section we\'ll find some good reasons why a method would
...
The name `!BaseClassName`{.interpreted-text role="class"} must be
defined in a namespace accessible from the scope containing the derived
...
- Use `isinstance`{.interpreted-text role="func"} to check an
  instance\'s type: `isinstance(obj, int)` will be `True` only if
  `obj.__class__` is `int`{.interpreted-text role="class"} or some class
  derived from `int`{.interpreted-text role="class"}.
- Use `issubclass`{.interpreted-text role="func"} to check class
  inheritance: `issubclass(bool, int)` is `True` since
  `bool`{.interpreted-text role="class"} is a subclass of
  `int`{.interpreted-text role="class"}. However,
  `issubclass(float, int)` is `False` since `float`{.interpreted-text
  role="class"} is not a subclass of `int`{.interpreted-text
  role="class"}.
...
`!DerivedClassName`{.interpreted-text role="class"}, it is searched for
in `!Base1`{.interpreted-text role="class"}, then (recursively) in the
base classes of `!Base1`{.interpreted-text role="class"}, and if it was
not found there, it was searched for in `!Base2`{.interpreted-text
role="class"}, and so on.
...
`super`{.interpreted-text role="func"}. This approach is known in some
...
bottommost class). For example, all classes inherit from
`object`{.interpreted-text role="class"}, so any case of multiple
inheritance provides more than one path to reach
`object`{.interpreted-text role="class"}. To keep the base classes from
being accessed more than once, the dynamic algorithm linearizes the
search order in a way that preserves the left-to-right ordering
specified in each class, that calls each parent only once, and that is
monotonic (meaning that a class can be subclassed without affecting the
precedence order of its parents). Taken together, these properties make
it possible to design reliable and extensible classes with multiple
inheritance. For more detail, see `python_2.3_mro`{.interpreted-text
role="ref"}.

In some cases multiple inheritance is not allowed; see
`multiple-inheritance`{.interpreted-text role="ref"} for details.
...
limited support for such a mechanism, called
`name mangling`{.interpreted-text role="dfn"}. Any identifier of the
...
stripped. This mangling is done without regard to the syntactic position
of the identifier, as long as it occurs within the definition of a
class.

::: seealso
The
`private name mangling specifications <private-name-mangling>`{.interpreted-text
role="ref"} for details and special cases.
:::
...
\"record\" or C \"struct\", bundling together a few named data items.
The idiomatic approach is to use `dataclasses`{.interpreted-text
role="mod"} for this purpose:
...
`~io.TextIOBase.read`{.interpreted-text role="meth"} and
`~io.TextIOBase.readline`{.interpreted-text role="meth"} that get the
data from a string buffer instead, and pass it as an argument.

`Instance method objects <instance-methods>`{.interpreted-text
role="ref"} have attributes, too:
`m.__self__ <method.__self__>`{.interpreted-text role="attr"} is the
instance object with the method `!m`{.interpreted-text role="meth"}, and
`m.__func__ <method.__func__>`{.interpreted-text role="attr"} is the
`function object <user-defined-funcs>`{.interpreted-text role="ref"}
corresponding to the method.

## Iterators {#tut-iterators}

By now you have probably noticed that most container objects can be
looped over using a `for`{.interpreted-text role="keyword"} statement:
...
`for`{.interpreted-text role="keyword"} statement calls
`iter`{.interpreted-text role="func"} on the container object. The
function returns an iterator object that defines the method
`~iterator.__next__`{.interpreted-text role="meth"} which accesses
elements in the container one at a time. When there are no more
elements, `~iterator.__next__`{.interpreted-text role="meth"} raises a
`StopIteration`{.interpreted-text role="exc"} exception which tells the
`!for`{.interpreted-text role="keyword"} loop to terminate. You can call
the `~iterator.__next__`{.interpreted-text role="meth"} method using the
`next`{.interpreted-text role="func"} built-in function; this example
...
add iterator behavior to your classes. Define an
`~container.__iter__`{.interpreted-text role="meth"} method which
returns an object with a `~iterator.__next__`{.interpreted-text
role="meth"} method. If the class defines `!__next__`{.interpreted-text
role="meth"}, then `!__iter__`{.interpreted-text role="meth"} can just
return `self`:
...
`Generators <generator>`{.interpreted-text role="term"} are a simple and
powerful tool for creating iterators. They are written like regular
functions but use the `yield`{.interpreted-text role="keyword"}
statement whenever they want to return data. Each time
`next`{.interpreted-text role="func"} is called on it, the generator
...
generators so compact is that the `~iterator.__iter__`{.interpreted-text
role="meth"} and `~generator.__next__`{.interpreted-text role="meth"}
...
`StopIteration`{.interpreted-text role="exc"}. In combination, these
features make it easy to create iterators with no more effort than
...
a special case, however, when provided as the sole argument to a
function (as in the examples involving `sum`, `set`, `max`, and `list`
above), the generator expression does not need to be wrapped in an
additional set of parentheses. That is to say, the following two pieces
of code are semantically equivalent:
...
[^1]: Except for one thing. Module objects have a secret read-only
    attribute called `~object.__dict__`{.interpreted-text role="attr"}
    which returns the dictionary used to implement the module\'s
    namespace; the name `__dict__` is an attribute but not a global
    name. Obviously, using this violates the abstraction of namespace
