...
are *public* (except see below `tut-private`), and all member functions are *virtual*. As in Modula-3,
...
(containing functions such as `abs`, and
...
a module called `__main__`, so they have
...
`nonlocal` statement can be used; if
...
and namespaces, and how `global` and
...
*scope_test*\'s binding of *spam*. The `nonlocal` assignment changed *scope_test*\'s binding of *spam*,
and the `global` assignment changed
...
definition header (`!ClassName` in the
example).
...
`MyClass.i` by assignment. `~type.__doc__` is also a valid attribute, returning the docstring
...
special method named `~object.__init__`,
like this:

    def __init__(self):
        self.data = []

When a class defines an `~object.__init__` method, class instantiation automatically invokes
...
Of course, the `~object.__init__` method
...
As discussed in `tut-object`, shared data
...
The name `!BaseClassName` must be
...
- Use `isinstance` to check an
  instance\'s type: `isinstance(obj, int)` will be `True` only if
  `obj.__class__` is `int` or some class
  derived from `int`.
...
`!DerivedClassName`, it is searched for
in `!Base1`, then (recursively) in the
base classes of `!Base1`, and if it was
...
`super`. This approach is known in some
...
`object`, so any case of multiple
inheritance provides more than one path to reach
`object`. To keep the base classes from
...
inheritance. For more detail, see `python_2.3_mro`.

In some cases multiple inheritance is not allowed; see
`multiple-inheritance` for details.
...
`name mangling`. Any identifier of the
...
::: seealso
The
`private name mangling specifications <private-name-mangling>` for details and special cases.
:::
...
`~io.TextIOBase.read` and
`~io.TextIOBase.readline` that get the
data from a string buffer instead, and pass it as an argument.

`Instance method objects <instance-methods>` have attributes, too:
`m.__self__ <method.__self__>` is the
instance object with the method `!m`, and
`m.__func__ <method.__func__>` is the
`function object <user-defined-funcs>`
...
`for` statement calls
`iter` on the container object. The
...
elements, `~iterator.__next__` raises a
`StopIteration` exception which tells the
`!for` loop to terminate. You can call
the `~iterator.__next__` method using the
`next` built-in function; this example
...
`~container.__iter__` method which
returns an object with a `~iterator.__next__` method. If the class defines `!__next__`, then `!__iter__` can just
return `self`:
...
`StopIteration`. In combination, these
...
[^1]: Except for one thing. Module objects have a secret read-only
    attribute called `~object.__dict__`
