...
::: {.module synopsis="Standard errno system symbols."}
errno
:::
...
descriptions are borrowed from `linux/include/errno.h`, which should be all-inclusive.

::: data
errorcode

Dictionary providing a mapping from the errno value to the string name
in the underlying system. For instance, `errno.errorcode[errno.EPERM]`
maps to `'EPERM'`.
:::
