...
::: {.module synopsis="Base classes for SAX event handlers."}
xml.sax.handler
:::
...
`!xml.sax.handler`, so that all methods
...
::: ErrorHandler
Interface used by the parser to present error and warning messages to
...
::: LexicalHandler
Interface used by the parser to represent low frequency events which may
not be of interest to many applications.
:::

In addition to these classes, `!xml.sax.handler` provides symbolic constants for the feature and property
names.
...
| true: All element names, prefixes, attribute names, Namespace URIs,
  and local names are interned using the built-in intern function.
...
Enabling opens a vulnerability to [external entity
...
your [threat model](https://en.wikipedia.org/wiki/Threat_model) before
...
| true: Include all external parameter entities, including the external
  DTD subset.
| false: Do not include any external parameter entities, even the
  external DTD subset.
| access: (parsing) read-only; (not parsing) read/write
:::
...
| description: An optional extension handler for DTD-related events
  other than notations and unparsed entities.
| access: read/write
:::
...
| description: When parsing, the current DOM node being visited if this
  is a DOM iterator; when not parsing, the root DOM node for iteration.
| access: (parsing) read-only; (not parsing) read/write
:::
...
## ContentHandler Objects {#content-handler-objects}

Users are expected to subclass `ContentHandler` to support their application. The following methods are
called by the parser on the appropriate events in the input document:
...
`endPrefixMapping` events are not
guaranteed to be properly nested relative to each-other: all
`startPrefixMapping` events will occur
before the corresponding `startElement`
event, and all `endPrefixMapping` events
...
`endPrefixMapping` events is not
...
as a string and the *attrs* parameter holds an object of the
`Attributes <attributes-objects>`
interface containing the attributes of the element. The object passed as
...
the attributes, use the `copy` method of
the *attrs* object.
:::
...
`(uri, localname)` tuple, the *qname* parameter contains the raw XML 1.0
name used in the source document, and the *attrs* parameter holds an
instance of the `AttributesNS <attributes-ns-objects>` interface containing the attributes of the element. If no
namespace is associated with the element, the *uri* component of *name*
will be `None`. The object passed as *attrs* may be re-used by the
...
The *name* parameter contains the name of the element type, just as with
the `startElementNS` method, likewise the
*qname* parameter.
:::
...
## DTDHandler Objects {#dtd-handler-objects}

`DTDHandler` instances provide the
following methods:
...
## EntityResolver Objects {#entity-resolver-objects}
...
## ErrorHandler Objects {#sax-error-handler}

Objects with this interface are used to receive error and warning
information from the `~xml.sax.xmlreader.XMLReader`. If you create an object that implements this interface,
then register the object with your
`~xml.sax.xmlreader.XMLReader`, the
parser will call the methods in your object to report all warnings and
errors. There are three levels of errors available: warnings, (possibly)
recoverable errors, and unrecoverable errors. All methods take a
`~xml.sax.SAXParseException` as the only
...
## LexicalHandler Objects {#lexical-handler-objects}
