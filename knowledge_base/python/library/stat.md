...
::: {.module synopsis="Utilities for interpreting the results of os.stat(),
...
functions for interpreting the results of `os.stat`, `os.fstat` and
`os.lstat` (if they exist). For complete
details about the `stat`,
`!fstat` and `!lstat` calls, consult the documentation for your system.

::: versionchanged
3.4 The stat module is backed by a C implementation.
:::

The `!stat` module defines the following
...
`os.chmod`\-\--that is, the file\'s
permission bits, plus the sticky bit, set-group-id, and set-user-id bits
...
here are useful when you are doing multiple tests of the same file and
wish to avoid the overhead of the `stat` system call for each test. These are also useful when
checking for information about a file that isn\'t handled by
`os.path`, like the tests for block and
character devices.
...
::: versionchanged
3.4 The function supports `S_IFDOOR`,
`S_IFPORT` and
`S_IFWHT`.
:::
:::::
...
call to `os.stat`,
`os.fstat`, or
`os.lstat`; this can sometimes be useful,
...
effective group ID of the creating process, and directories created
there will also get the `S_ISGID` bit
set. For a file that does not have the group execution bit
(`S_IXGRP`) set, the set-group-ID bit
indicates mandatory file/record locking (see also
`S_ENFMT`).
:::
...
`S_ISGID`: file/record locking is
enforced on files that do not have the group execution bit
(`S_IXGRP`) set.
:::
...
`os.stat`. See the [Windows API
documentation](https://msdn.microsoft.com/en-us/library/windows/desktop/gg258117.aspx)
...
the `st_reparse_tag` member returned by `os.lstat`. These are well-known constants, but are not an exhaustive
list.
...
On Linux, the following file attribute constants are available for use
when testing bits in the
`~os.statx_result.stx_attributes` and
`~os.statx_result.stx_attributes_mask`
members returned by `os.statx`. See the
`statx(2)` man page for more detail on
the meaning of these constants.

:::: data
STATX_ATTR_COMPRESSED STATX_ATTR_IMMUTABLE STATX_ATTR_APPEND
STATX_ATTR_NODUMP STATX_ATTR_ENCRYPTED STATX_ATTR_AUTOMOUNT
STATX_ATTR_MOUNT_ROOT STATX_ATTR_VERITY STATX_ATTR_DAX
