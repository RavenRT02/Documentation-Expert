...
::: {.module synopsis="Trace or track Python statement execution."}
trace
:::

**Source code:** `Lib/trace.py`

The `!trace` module allows you to trace
program execution, generate annotated statement coverage listings, print
caller/callee relationships and list functions executed during a program
run. It can be used in another program or from the command line.
...
## Command-Line Usage {#trace-cli}

The `!trace` module can be invoked from
the command line. It can be as simple as :

    python -m trace --count -C . somefile.py ...

The above will execute `somefile.py` and
generate annotated listings of all Python modules imported during the
...
::: versionadded
3.8 Added `--module` option that allows running an executable module.
:::
...
`!trace`. The
`--listfuncs <-l>` option is mutually
exclusive with the `--trace <-t>` and
`--count <-c>` options. When
`--listfuncs <-l>` is provided, neither
`--count <-c>` nor
`--trace <-t>` are accepted, and vice
versa.
...
`/{package}/{module}.cover`{.interpreted-text role="file"}.
:::
...
When using `--count <-c>` or
`--report <-r>`, write a brief summary
to stdout for each file processed.
:::
...
several runs with `--count <-c>`, and
then produce a single set of annotated listings at the end.
:::
...
## Programmatic Interface {#trace-api}
...
*trace* enables line execution tracing. *countfuncs* enables listing of
the functions called during the run. *countcallers* enables call
...
`Trace` object with the current tracing
parameters.
:::
...
Return a `CoverageResults` object that
contains the cumulative results of all previous calls to `run`, `runctx`
and `runfunc` for the given `Trace`
