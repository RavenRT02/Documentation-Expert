...
the `select` module primitives. Users are
encouraged to use this module instead, unless they want precise control
over the OS-level primitives used.

It defines a `BaseSelector` abstract
base class, along with several concrete implementations
(`KqueueSelector`,
`EpollSelector`\...), that can be used
to wait for I/O readiness notification on multiple file objects. In the
following, \"file object\" refers to any object with a
`~io.IOBase.fileno` method, or a raw file
descriptor. See `file object`.

`DefaultSelector` is an alias to the
most efficient implementation available on the current platform: this
...
::::::: SelectorKey
A `SelectorKey` is a
`~collections.namedtuple` used to
associate a file object to its underlying file descriptor, selected
event mask and attached data. It is returned by several
`BaseSelector` methods.
...
::::::::::::: BaseSelector
A `BaseSelector` is used to wait for I/O
event readiness on multiple file objects. It supports file stream
registration, unregistration, and a method to wait for I/O events on
those streams, with an optional timeout. It\'s an abstract base class,
so cannot be instantiated. Use `DefaultSelector` instead, or one of `SelectSelector`, `KqueueSelector` etc. if
you want to specifically use an implementation, and your platform
supports it. `BaseSelector` and its
...
instance, or raises a `ValueError` in case
of invalid event mask or file descriptor, or
`KeyError` if the file object is already
registered.
:::
...
This returns the associated `SelectorKey` instance, or raises a `KeyError` if *fileobj* is not registered. It will raise
`ValueError` if *fileobj* is invalid (e.g.
it has no `fileno()` method or its `fileno()` method has an invalid
return value).
:::
...
instance, or raises a `ValueError` in case
of invalid event mask or file descriptor, or
`KeyError` if the file object is not
registered.
:::
...
*key* is the `SelectorKey` instance
corresponding to a ready file object. *events* is a bitmask of events
...
(see `475` for the rationale), instead of
returning an empty list of events before the timeout.
:::
::::::
...
This returns the `SelectorKey` instance
associated to this file object, or raises `KeyError` if the file object is not registered.
:::
...
This returns a `~collections.abc.Mapping` instance mapping registered file objects to their
