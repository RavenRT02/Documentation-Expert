...
The `!bdb` module handles basic debugger
functions, like setting breakpoints or managing execution via the
debugger.
...
Exception raised by the `Bdb` class for
quitting the debugger.
:::

The `!bdb` module also defines two
classes:

::::::::::::::::::: {.Breakpoint(self, .file, .line, .temporary=False, .cond=None, .funcname=None)}
This class implements temporary breakpoints, ignore counts, disabling
and (re-)enabling, and conditionals.

Breakpoints are indexed by number through a list called
`bpbynumber` and by `(file, line)` pairs
through `bplist`. The former points to a
...
When creating a breakpoint, its associated
`file name <file>` should be in canonical
form. If a `funcname` is defined, a
breakpoint `hit <hits>` will be counted
when the first line of that function is executed. A
`conditional <cond>` breakpoint always
counts a `hit <hits>`.

`Breakpoint` instances have the
following methods:
...
Print the output of `bpformat` to the
file *out*, or if it is `None`, to standard output.
:::
...
Line number of the `Breakpoint` within
`file`.
:::
...
`True` if a `Breakpoint` at (file, line)
is temporary.
:::
...
Condition for evaluating a `Breakpoint`
at (file, line).
:::
...
Dictionary of `Breakpoint` instances
indexed by (`file`,
`line`) tuples.
:::
...
The `Bdb` class acts as a generic Python
debugger base class.
...
The *backend* argument specifies the backend to use for
`Bdb`. It can be either `'settrace'` or
`'monitoring'`. `'settrace'` uses `sys.settrace` which has the best backward compatibility. The
`'monitoring'` backend uses the new `sys.monitoring` that was introduced in Python 3.12, which can be much more
efficient because it can disable unused events. We are trying to keep
the exact interfaces for both backends, but there are some differences.
The debugger developers are encouraged to use the `'monitoring'` backend
...
The following methods of `Bdb` normally
...
`case-normalized <os.path.normcase>`
`absolute path
<os.path.abspath>`. A *filename* with
angle brackets, such as `"<stdin>"` generated in interactive mode, is
returned unchanged.
:::
...
`!stopframe`,
`!returnframe` and
`quitting <Bdb.set_quit>` attributes with
...
for more information on the trace function. For more information on code
...
`user_line` method (which should be
overridden in subclasses). Raise a `BdbQuit` exception if the
`quitting  <Bdb.set_quit>` flag is set
(which can be set from `user_line`).
Return a reference to the `trace_dispatch` method for further tracing in that scope.
:::
...
`user_call` method (which should be
overridden in subclasses). Raise a `BdbQuit` exception if the
`quitting  <Bdb.set_quit>` flag is set
...
`user_return` method (which should be
overridden in subclasses). Raise a `BdbQuit` exception if the
`quitting  <Bdb.set_quit>` flag is set
...
`user_exception` method (which should be
overridden in subclasses). Raise a `BdbQuit` exception if the
`quitting  <Bdb.set_quit>` flag is set
...
Delete temporary breakpoints based on information from
`effective`.
:::
...
Called from `dispatch_call` if a break
might stop inside the called function.
...
Called from `dispatch_line` when either
`stop_here` or
...
Called from `dispatch_return` when
`stop_here` returns `True`.
:::

::: method
user_exception(frame, exc_info)

Called from `dispatch_exception` when
`stop_here` returns `True`.
:::
...
This raises `BdbQuit` in the next call to
one of the `!dispatch_\*` methods.
::::
...
`Breakpoint.bpbynumber`. If *arg* is not
numeric or out of range, return an error message.
:::
...
Derived classes and clients can call the following methods to disable
and restart events to achieve better performance. These methods only
work when using the `'monitoring'` backend.

:::: method
disable_current_event()

Disable the current event until the next time
`restart_events` is called. This is
helpful when the debugger is not interested in the current line.
...
Restart all the disabled events. This function is automatically called
in `dispatch_*` methods after `user_*` methods are called. If the
...
Debug a statement executed via the `exec`
function. *globals* defaults to `!__main__.__dict__`, *locals* defaults to *globals*.
:::

::: method
runeval(expr, globals=None, locals=None)

Debug an expression executed via the `eval` function. *globals* and *locals* have the same meaning as
in `run`.
:::
...
`b.line <bdb.Breakpoint.line>` is the
same as the one in *frame*. If the breakpoint was set via
`function name <bdb.Breakpoint.funcname>`, we have to check we are in the right *frame* (the right
function) and if we are on its first executable line.
:::
...
(`file <bdb.Breakpoint.file>`,
`line <bdb.Breakpoint.line>`) (which must
exist) that is `enabled <bdb.Breakpoint.enabled>`, for which `checkfuncname`
is true, and that has neither a false
`condition <bdb.Breakpoint.cond>` nor
positive `ignore <bdb.Breakpoint.ignore>`
count. The *flag*, meaning that a temporary breakpoint should be
deleted, is `False` only when the
`cond <bdb.Breakpoint.cond>` cannot be
evaluated (in which case,
`ignore <bdb.Breakpoint.ignore>` count is
ignored).
...
Start debugging with a `Bdb` instance
from caller\'s frame.
:::