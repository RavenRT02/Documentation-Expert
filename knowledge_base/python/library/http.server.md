...
::: {.module synopsis="HTTP server and request handlers."}
http.server
:::
...
`!http.server` is not recommended for
production. It only implements
...
One class, `HTTPServer`, is a
`socketserver.TCPServer` subclass. It
...
This class builds on the `~socketserver.TCPServer` class by storing the server address as instance variables
named `server_name` and
`server_port`. The server is accessible
by the handler, typically through the handler\'s
`server` instance variable.
:::

:::: {.ThreadingHTTPServer(server_address, .RequestHandlerClass)}
This class is identical to HTTPServer but uses threads to handle
requests by using the `~socketserver.ThreadingMixIn`. This is useful to handle web browsers pre-opening
sockets, on which `HTTPServer` would
wait indefinitely.
...
::::: {.HTTPSServer(server_address, .RequestHandlerClass,\\ .bind_and_activate=True, .*, .certfile, .keyfile=None,\\ .password=None, .alpn_protocols=None)}
Subclass of `HTTPServer` with a wrapped
socket using the `ssl` module. If the
`ssl` module is not available,
instantiating a `!HTTPSServer` object
...
PKCS#8, but beware that this could possibly expose hardcoded passwords
in clear.

::: seealso
See `ssl.SSLContext.load_cert_chain` for
additional information on the accepted values for *certfile*, *keyfile*
and *password*.
:::

When specified, the *alpn_protocols* argument must be a sequence of
strings specifying the \"Application-Layer Protocol Negotiation\" (ALPN)
protocols supported by the server. ALPN allows the server and the client
to negotiate the application protocol during the TLS handshake.
...
:::: {.ThreadingHTTPSServer(server_address, .RequestHandlerClass,\\ .bind_and_activate=True, .*, .certfile, .keyfile=None,\\ .password=None, .alpn_protocols=None)}
This class is identical to `HTTPSServer`
but uses threads to handle requests by inheriting from
`~socketserver.ThreadingMixIn`. This is
analogous to `ThreadingHTTPServer` only
using `HTTPSServer`.
...
`ThreadingHTTPServer`,
`HTTPSServer` and
`ThreadingHTTPSServer` must be given a
...
must be subclassed to handle each request method (for example, `'GET'`
or `'POST'`). `BaseHTTPRequestHandler`
...
`!do_SPAM` method will be called with no
arguments. All of the relevant information is stored in instance
variables of the handler. Subclasses should not need to override or
extend the `!__init__` method.

`BaseHTTPRequestHandler` has the
...
Boolean that should be set before `handle_one_request` returns, indicating if another request may be expected, or
if the connection should be shut down.
:::
...
then `path` includes the query. Using the terminology of
`3986`, `path` here includes `hier-part`
and the `query`.
:::
...
`MessageClass` class variable. This
instance parses and manages the headers in the HTTP request. The
`~http.client.parse_headers` function
from `http.client` is used to parse the
headers and it requires that the HTTP request provide a valid
`5322` style header.
:::
...
An `io.BufferedIOBase` input stream,
ready to read from the start of the optional input data.
:::
...
::: versionchanged
3.6 This is an `io.BufferedIOBase`
stream.
:::
::::
...
`version_string` method and the
`server_version` class variable. For
example, `'Python/1.4'`.
:::
...
`send_error` method for building an error
response to the client. The string is filled by default with variables
from `responses` based on the status code
...
then include an accurate `Content-Length` header (using
`send_header`) in all of its responses to
clients. For backwards compatibility, the setting defaults to
`'HTTP/1.0'`.
:::
...
Specifies an `email.message.Message`-like class to parse HTTP headers. Typically, this is not
overridden, and it defaults to
...
*explain* key. It is used by `send_response_only` and `send_error` methods.
:::
...
Calls `handle_one_request` once (or, if
persistent connections are enabled, multiple times) to handle incoming
HTTP requests. You should never need to override it; instead, implement
...
`!do_\*` method. You should never need to
override it.
:::
...
server does not want the client to continue. For example, the server can
choose to send `417 Expectation Failed` as a response header and
`return False`.
...
formatted using the `error_message_format` attribute and emitted, after a complete set of headers, as
the response body. The `responses`
...
or the response code is one of the following: `1{xx}`, `204 No Content`, `205 Reset Content`, `304 Not Modified`.
...
headers using the `send_header` method,
then `send_response` should be followed
by an `end_headers` call.

::: versionchanged
3.3 Headers are stored to an internal buffer and
`end_headers` needs to be called
explicitly.
:::
::::
...
or `flush_headers` is invoked. *keyword*
should specify the header keyword, with *value* specifying its value.
Note that, after the send_header calls are done,
`end_headers` MUST BE called in order to
...
response) to the headers buffer and calls
`flush_headers`.
...
the message to `log_message`, so it takes
the same arguments (*format* and additional values).
:::
...
`log_message` are applied as inputs to
the formatting. The client ip address and current date and time are
...
in the format returned by `time.time`),
formatted for a message header. If *timestamp* is omitted, it uses the
...
::::::::::::::: {.SimpleHTTPRequestHandler(request, .client_address, .server, .\\ .*, .directory=None, .extra_response_headers=None)}
...
::: versionchanged
3.7 Added the *directory* parameter.
:::

::: versionchanged
3.9 The *directory* parameter accepts a
`path-like object`.
:::

::: versionchanged
3.15 Added *extra_response_headers* parameter.
:::

A lot of the work, such as parsing the request, is done by the base
class `BaseHTTPRequestHandler`. This
class implements the `do_GET` and
`do_HEAD` functions.
...
be guessed from the file extension of the requested URL. By default, it
...
Headers that the server sends automatically such as `Content-Type` will
not be overwritten by `!extra_response_headers`.
:::
...
`do_GET` method for a more complete
...
generated by calling the `list_directory`
method. This method uses `os.listdir` to
scan the directory, and returns a `404` error response if the
`~os.listdir` fails.

If the request was mapped to a file, it is opened. Any
`OSError` exception in opening the
requested file is mapped to a `404`, `'File not found'` error. If there
was an `'If-Modified-Since'` header in the request, and the file was not
modified after this time, a `304`, `'Not Modified'` response is sent.
Otherwise, the content type is guessed by calling the
`guess_type` method, which in turn uses
the *extensions_map* variable, and the file contents are returned.

A `'Content-type:'` header with the guessed content type is output,
followed by a `'Content-Length:'` header with the file\'s size and a
`'Last-Modified:'` header with the file\'s modification time.

The instance attribute `extra_response_headers` is a sequence of `(name, value)` pairs containing
user-defined extra response headers.
...
The `SimpleHTTPRequestHandler` class can
be used in the following manner in order to create a very basic
...
`SimpleHTTPRequestHandler` can also be
subclassed to enhance behavior, such as using different index file names
by overriding the class attribute `index_pages`.

## Command-line interface {#http-server-cli}

`!http.server` can also be invoked
directly using the `-m` switch of the
interpreter. The following example illustrates how to serve files
...
::: versionchanged
3.8 Support IPv6 in the `--bind` option.
:::
:::::
...
the server uses the current directory. For example, the following
...
Specify an additional extra HTTP Response Header to send on successful
HTTP 200 responses. Can be used multiple times to send additional custom
response headers. Headers that are sent automatically by the server (for
instance Content-Type) will not be overwritten by the server.
...
`SimpleHTTPRequestHandler` will follow
...
`BaseHTTPRequestHandler.send_response_only` assume sanitized input and do not perform input validation
such as checking for the presence of CRLF sequences. Untrusted input may
result in HTTP Header injection attacks.
...
::: versionchanged
3.12 Control characters are scrubbed in stderr logs.
:::