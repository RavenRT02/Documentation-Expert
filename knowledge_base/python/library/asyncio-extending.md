...
The main direction for `asyncio` extending
is writing custom *event loop* classes. Asyncio has helpers that could
be used to simplify this task.
...
`asyncio.AbstractEventLoop` declares
very many methods. Implementing all them from scratch is a tedious job.
...
declared but not implemented in
...
`asyncio.Future` and
`asyncio.Task` should be never created
directly, please use corresponding
`loop.create_future` and
`loop.create_task`, or
...
keep a task visible by `asyncio.all_tasks` and `asyncio.current_task`:
