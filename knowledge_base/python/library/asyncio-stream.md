...
::::::::: {.function async=""}
open_connection(host=None, port=None, \*, limit=None, ssl=None,
...
`StreamReader` instance. By default the
*limit* is set to 64 KiB.
...
`StreamWriter` created. To close the
socket, call its `~asyncio.StreamWriter.close` method.
::::

::: versionchanged
3.7 Added the *ssl_handshake_timeout* parameter.
:::

::: versionchanged
3.8 Added the *happy_eyeballs_delay* and *interleave* parameters.
:::
...
::: versionchanged
3.11 Added the *ssl_shutdown_timeout* parameter.
:::
:::::::::

::::::::: {.function async=""}
start_server(client_connected_cb, host=None, port=None, \*, limit=None,
...
arguments, instances of the `StreamReader` and `StreamWriter`
classes.

*client_connected_cb* can be a plain callable or a
`coroutine function <coroutine>`; if it is
a coroutine function, it will be automatically scheduled as a
`Task`.
...
::: versionchanged
3.7 Added the *ssl_handshake_timeout* and *start_serving* parameters.
:::
...
::: versionchanged
3.13 Added the *keep_alive* parameter.
:::
:::::::::
...
::::::::: {.function async=""}
open_unix_connection(path=None, \*, limit=None, ssl=None, sock=None,
server_hostname=None, ssl_handshake_timeout=None,
...
Similar to `open_connection` but operates
on Unix sockets.
...
:::::::::: {.function async=""}
start_unix_server(client_connected_cb, path=None, \*, limit=None,
...
Similar to `start_server` but works with
Unix sockets.
...
::: versionchanged
3.13 Added the *cleanup_socket* parameter.
:::
::::::::::
...
stream. As an `asynchronous iterable`,
the object supports the `async for`
statement.
...
read `bytes`. If EOF was received and
the internal buffer is empty, return an empty `bytes` object.
...
Raise an `IncompleteReadError` if EOF is
reached before *n* can be read. Use the
`IncompleteReadError.partial` attribute
to get the partially read data.
:::
...
`LimitOverrunError` exception is raised,
...
`IncompleteReadError` exception is raised,
and the internal buffer is reset. The
`IncompleteReadError.partial` attribute
...
suffix. For the purposes of `LimitOverrunError`, the shortest possible separator is considered to be the one
that matched.
...
The *separator* parameter may now be a `tuple` of separators.
:::
:::::
...
`BaseTransport.get_extra_info` for
details.
:::
...
::::: {.method async=""}
start_tls(sslcontext, \*, server_hostname=None,
...
::: versionchanged
3.12 Added the *ssl_shutdown_timeout* parameter.
:::
:::::
...
Should be called after `close` to wait
until the underlying connection is closed, ensuring that all data has
...
### TCP echo client using streams {#asyncio-tcp-echo-client-streams}
...
::: seealso
The
`TCP echo client protocol <asyncio_example_tcp_echo_client_protocol>` example uses the low-level
`loop.create_connection` method.
:::

### TCP echo server using streams {#asyncio-tcp-echo-server-streams}
...
::: seealso
The
`TCP echo server protocol <asyncio_example_tcp_echo_server_protocol>` example uses the `loop.create_server` method.
:::
...
### Register an open socket to wait for data using streams {#asyncio_example_create_connection-streams}
