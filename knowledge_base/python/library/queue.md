...
::: {.module synopsis="A synchronized queue class."}
queue
:::
...
multi-producer, multi-consumer queues. It is especially useful in
threaded programming when information must be exchanged safely between
multiple threads. The `Queue` class in
...
order in which the entries are retrieved. In a
`FIFO (first-in, first-out)` queue, the
first tasks added are the first retrieved. In a
`LIFO (last-in, first-out)` queue, the
most recently added entry is the first retrieved (operating like a
stack). With a priority queue, the entries are kept sorted (using the
`heapq` module) and the lowest valued
...
`SimpleQueue`, whose specific
implementation provides additional guarantees in exchange for the
smaller functionality.

The `!queue` module defines the following
classes and exceptions:

::: Queue(maxsize=0)
Constructor for a `FIFO (first-in, first-out)` queue. *maxsize* is an integer that sets the upperbound
limit on the number of items that can be placed in the queue. Insertion
will block once this size has been reached, until queue items are
consumed. If *maxsize* is less than or equal to zero, the queue size is
infinite.
:::

::: LifoQueue(maxsize=0)
Constructor for a `LIFO (last-in, first-out)` queue. *maxsize* is an integer that sets the upperbound
limit on the number of items that can be placed in the queue. Insertion
will block once this size has been reached, until queue items are
consumed. If *maxsize* is less than or equal to zero, the queue size is
infinite.
...
is the one that would be returned by `min(entries)`). A typical pattern
...
:::: SimpleQueue()
Constructor for an unbounded
`FIFO (first-in, first-out)` queue.
Simple queues lack advanced functionality such as task tracking.

Simple queues are `generic <generics>`
over the type of their items.
...
Exception raised when non-blocking `~Queue.get` (or `~Queue.get_nowait`) is
called on a `Queue` object which is
empty.
:::
...
Exception raised when non-blocking `~Queue.put` (or `~Queue.put_nowait`) is
called on a `Queue` object which is
full.
:::
...
`~Queue.get` is called on a
`Queue` object which has been shut down.
...
## Queue Objects {#queueobjects}

Queue objects (`Queue`,
`LifoQueue`, or
`PriorityQueue`) provide the public
...
*timeout* seconds and raises the `Full`
...
Raises `ShutDown` if the queue has been
shut down.
:::
...
*timeout* seconds and raises the `Empty`
...
the `Empty` exception (*timeout* is
...
Raises `ShutDown` if the queue has been
shut down and is empty, or if the queue has been shut down immediately.
:::
...
consumer threads. For each `get` used to
...
When no longer needed, `Queue` objects
can be wound down until empty or terminated immediately with a hard
shutdown.
...
`~Queue.put` raise
`ShutDown`. Currently blocked callers of
`~Queue.put` will be unblocked and will
raise `ShutDown` in the formerly blocked
thread.
...
unblocked and will raise `ShutDown`
...
`SimpleQueue` objects provide the public
methods described below.
...
provided for compatibility with `Queue.put`.

::: impl-detail
This method has a C implementation which is reentrant. That is, a
`put()` or `get()` call can be interrupted by another `put()` call in
the same thread without deadlocking or corrupting internal state inside
the queue. This makes it appropriate for use in destructors such as
`__del__` methods or `weakref` callbacks.
:::
::::
