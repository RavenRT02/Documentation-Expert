...
::: {.module synopsis="Exception classes raised by urllib.request."}
urllib.error
:::
...
exception classes for exceptions raised by
`urllib.request`. The base exception class
is `URLError`.
...
into a problem. It is a subclass of `OSError`.
...
::: versionchanged
3.3 `URLError` used to be a subtype of
...
function as a non-exceptional file-like return value (the same thing
...
An HTTP status code as defined in `2616`.
...
detects that the amount of the downloaded data is less than the expected
