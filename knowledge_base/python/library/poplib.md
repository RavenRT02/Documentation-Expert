...
::: {.module synopsis="POP3 protocol client (requires sockets)."}
poplib
:::
...
which encapsulates a connection to a POP3 server and implements the
protocol as defined in `1939`. The
`POP3` class supports both the minimal
and optional command sets from `1939`. The
`POP3` class also supports the `STLS`
command introduced in `2595` to enable
encrypted communication on an already established connection.

Additionally, this module provides a class `POP3_SSL`, which provides support for connecting to POP3 servers
...
::: audit-event
poplib.connect self,host,port poplib.POP3
:::

::: audit-event
poplib.putline self,line poplib.POP3

All commands will raise an `auditing event <auditing>` `poplib.putline` with arguments `self` and `line`, where
...
::::::::: {.POP3_SSL(host, .port=POP3_SSL_PORT, .*, .timeout=None, .context=None)}
This is a subclass of `POP3` that
connects to the server over an SSL encrypted socket. If *port* is not
specified, 995, the standard POP3-over-SSL port is used. *timeout* works
as in the `POP3` constructor. *context*
is an optional `ssl.SSLContext` object
which allows bundling SSL configuration options, certificates and
private keys into a single (potentially long-lived) structure. Please
read `ssl-security` for best practices.

::: audit-event
poplib.connect self,host,port poplib.POP3_SSL
:::

::: audit-event
poplib.putline self,line poplib.POP3_SSL
...
`socket` module are not caught). The
reason for the exception is passed to the constructor as a string.
:::
...
: The FAQ for the `fetchmail` POP/IMAP
  client collects information on POP3 server variations and RFC
  noncompliance that may be useful if you need to write an application
...
A `POP3` instance has the following
methods:
...
`2449`. Returns a dictionary in the form
`{'name': ['param'...]}`.
...
the mailbox on the server is locked until `~POP3.quit` is called.
:::
...
`(response, ['mesg_num octets', ...], octets)`. If *which* is set, it is
the message to list.
:::
...
Flag message number *which* for deletion. On most servers deletions are
not actually performed until QUIT (the major exception is Eudora QPOP,
...
the header of message number *which*. Result is in form
`(response, ['line', ...], octets)`.

The POP3 TOP command this method uses, unlike the RETR command, doesn\'t
set the message\'s seen flag; unfortunately, TOP is poorly specified in
...
contains the unique id for that message in the form
`'response mesgnum uid`, otherwise result is list
...
raises `error_proto` if not. Specified
in [RFC 6856](http://www.faqs.org/rfcs/rfc6856.html "RFC 6856").
...
`2595`. This is only allowed before user
authentication

*context* parameter is a `ssl.SSLContext` object which allows bundling SSL configuration options,
certificates and private keys into a single (potentially long-lived)
structure. Please read `ssl-security` for
best practices.
...
Instances of `POP3_SSL` have no
additional methods. The interface of this subclass is identical to its
parent.
