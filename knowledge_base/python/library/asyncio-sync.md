...
of the `threading` module with two
important caveats:
...
- methods of these synchronization primitives do not accept the
  *timeout* argument; use the `asyncio.wait_for` function to perform operations with timeouts.
...
When more than one coroutine is blocked in `acquire` waiting for the lock to be unlocked, only one coroutine
eventually proceeds.

Acquiring a lock is *fair*: the coroutine that proceeds will be the
first coroutine that started waiting on the lock.
:::
...
*false* with the `clear` method. The
...
Subsequent tasks awaiting on `~Event.wait` will now block until the `~Event.set` method is called again.
:::
...
`Event` and a `Lock`. It is possible to have multiple Condition objects share
one Lock, which allows coordinating exclusive access to a shared
resource between different tasks interested in particular states of that
shared resource.

The optional *lock* argument must be a `Lock` object or `None`. In the latter case a new Lock object is
...
shortly after. If called with an *unlocked* lock a
...
This method acts like `notify`, but wakes
up all waiting tasks.
...
`notify_all` call. Once awakened, the
Condition re-acquires its lock and this method returns `True`.

Note that a task *may* return from this call spuriously, which is why
the caller should always re-check the state and be prepared to
`~Condition.wait` again. For this reason,
you may prefer to use `~Condition.wait_for` instead.
:::
...
boolean value. The method will repeatedly
`~Condition.wait` until the predicate
...
`release` call. The counter can never go
below zero; when `acquire` finds that it
is zero, it blocks, waiting until some task calls
`release`.
...
return `True` immediately. If it is zero, wait until a
...
Bounded Semaphore is a version of `Semaphore` that raises a `ValueError`
in `~Semaphore.release` if it increases
...
`~Barrier.wait` method and would be
blocked until the specified number of tasks end up waiting on
`~Barrier.wait`. At that point all of the
waiting tasks would unblock simultaneously.

`async with` can be used as an
alternative to awaiting on `~Barrier.wait`.
...
is waiting. It could raise a `CancelledError` if a task is cancelled.
:::
...
will receive the `BrokenBarrierError`
exception.
...
calls to `~Barrier.wait` to fail with the
`BrokenBarrierError`. Use this for
example if one of the tasks needs to abort, to avoid infinite waiting
tasks.
:::
...
`with` statement (`with await lock`,
`with (yield from lock)`) was removed. Use `async with lock` instead.
:::