...
available elsewhere in the standard library (for example,
`getopt` covers the very specific task of
\"mimic the C `!getopt` API in
...
are `soft deprecated` and their use is
actively discouraged in new projects. With the removal of various
obsolete modules through `594`, there are
currently no modules in this latter category.

::: {.toctree maxdepth="1"}
getopt.rst profile.rst
:::