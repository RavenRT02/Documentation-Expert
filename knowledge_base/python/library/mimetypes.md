...
::: {.module synopsis="Mapping of filename extensions to MIME types."}
mimetypes
:::

**Source code:** `Lib/mimetypes.py`

The `!mimetypes` module converts between a
filename or URL and the MIME type associated with the filename
extension. Conversions are provided from filename to MIME type and from
MIME type to filename extension; encodings are not supported for the
latter conversion.
...
`init` if they rely on the information
`init` sets up.
...
*url*. URL can be a string or a `path-like object`.

The return value is a tuple `(type, encoding)` where *type* is `None` if
the type can\'t be guessed (missing or unknown suffix) or a string of
the form `'type/subtype'`, usable for a MIME
`content-type` header.

*encoding* is `None` for no encoding or the name of the program used to
encode (e.g. `compress` or
`gzip`). The encoding is suitable for
use as a `Content-Encoding` header,
**not** as a `Content-Transfer-Encoding` header. The mappings are table driven. Encoding
suffixes are case-sensitive. Suffix mappings and type suffixes are first
...
operating system. Only file types recognized by the OS or explicitly
registered with Python\'s internal database can be identified. When
*strict* is `True` (the default), only the IANA types are supported;
when *strict* is `False`, some additional non-standard but commonly used
MIME types are also recognized.

::: versionchanged
3.8 Added support for *url* being a `path-like object`.
:::

::: soft-deprecated
3.13 Passing a file path instead of URL. Use
...
the `guess_type` function, but accepts a
...
extensions, including the leading dot (`'.'`). The extensions are not
guaranteed to have been associated with any particular data stream, but
would be mapped to the MIME type *type* by
`guess_type` and
`guess_file_type`.
...
MIME type *type* by `guess_type` and
`guess_file_type`. If no extension can be
...
map. If omitted, the file names to use are taken from
`knownfiles`; on Windows, the current
registry settings are loaded. Each file named in *files* or
`knownfiles` takes precedence over those
named before it. Calling `init`
...
If *files* is `None` the internal data structure is completely rebuilt
to its initial default value. This is a stable operation and will
produce the same results when called multiple times.

::: versionchanged
3.2 Previously, Windows registry settings were ignored.
:::
::::
...
Load the type map given in the file *filename*, if it exists. The type
map is returned as a dictionary mapping filename extensions, including
the leading dot (`'.'`), to strings of the form `'type/subtype'`. If the
...
known extensions. Valid extensions are empty or start with a `'.'`.
...
::: deprecated
3.14 *ext* values that do not start with `'.'` are deprecated.
:::

::: versionchanged
next *ext* now must start with `'.'`. Otherwise
...
typically named `mime.types` and are
...
`.tgz` extension is mapped to
`.tar.gz` to allow the encoding and type
...
The `MimeTypes` class may be useful for
applications which may want more than one MIME-type database; it
provides an interface similar to the one of the
`!mimetypes` module.

:::::::::::::::::::: {.MimeTypes(filenames=(), .strict=True)}
This class represents a MIME-types database. By default, it provides
access to the same database as the rest of this module. The initial
database is a copy of that provided by the module, and may be extended
by loading additional `mime.types`-style
files into the database using the `read`
or `readfp` methods. The mapping
...
initially a copy of the global `encodings_map` defined in the module.
:::
...
types: the first dictionary is for the non-standards types and the
...
filename extensions: the first dictionary is for the non-standards types
...
Similar to the `guess_extension`
...
Similar to the `guess_type` function,
...
Similar to the `guess_file_type`
function, using the tables stored as part of the object.
...
Similar to the `guess_all_extensions`
function, using the tables stored as part of the object.
:::
...
the format of the standard `mime.types`
files.
...
extensions start with a \'.\' or are empty. When the extension is
already known, the new type will replace the old one. When the type is
already known the extension will be added to the list of known
extensions.
...
## Command-line usage {#mimetypes-cli}

The `!mimetypes` module can be executed as
...
`!mimetypes` command-line interface:
