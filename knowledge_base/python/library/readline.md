...
::: {.module synopsis="GNU readline support for Python."}
readline
:::

The `!readline` module defines a number of
functions to facilitate completion and reading/writing of history files
from the Python interpreter. This module can be used directly, or via
the `rlcompleter` module, which supports
completion of Python identifiers at the interactive prompt. Settings
...
typically `.inputrc` in your home directory. See [Readline Init
File](https://tiswww.cwru.edu/php/chet/readline/rluserman.html#Readline-Init-File)
...
(`libedit`) library instead of GNU readline. On macOS the
`!readline` module detects which library
is being used at run time.
...
`backend` to determine which library is
being used.
...
`!rl_parse_and_bind` in the underlying
library.
:::
...
filename used. This calls `!rl_read_init_file` in the underlying library. It raises an
`auditing event <auditing>` `open` with
the file name if given, and `"<readline_init_file>"` otherwise,
regardless of which file the library resolves.

::: versionchanged
3.14 The auditing event was added.
:::
::::
...
(`!rl_line_buffer` in the underlying
library).
:::
...
`!rl_insert_text` in the underlying
library, but ignores the return value.
:::
...
of the line buffer. This calls `!rl_redisplay` in the underlying library.
:::
...
default filename is `~/.history`. This
calls `!read_history` in the underlying
library and raises an `auditing event <auditing>` `open` with the file name if given and `"~/.history"`
otherwise.
...
existing file. The default filename is `~/.history`. This calls `!write_history` in the underlying library and raises an
`auditing event <auditing>` `open` with
...
filename is `~/.history`. The file must
already exist. This calls `!append_history` in the underlying library. This function only exists if
Python was compiled for a version of the library that supports it. It
...
::: function
get_history_length() set_history_length(length)

Set or return the desired number of lines to save in the history file.
The `write_history_file` function uses
this value to truncate the history file, by calling
...
from `get_history_length`, which returns
...
is one-based. This calls `!history_get`
...
position is zero-based. This calls `!remove_history` in the underlying library.
:::
...
is zero-based. This calls `!replace_history_entry` in the underlying library.
:::
...
should be a Boolean value that when true, enables auto history, and that
when false, disables auto history.
...
`!rl_startup_hook` callback of the
underlying library. If *function* is specified, it will be used as the
new hook function; if omitted or `None`, any function already installed
is removed. The hook is called with no arguments just before readline
...
Set or remove the function invoked by the
`!rl_pre_input_hook` callback of the
underlying library. If *function* is specified, it will be used as the
new hook function; if omitted or `None`, any function already installed
is removed. The hook is called with no arguments after the first prompt
has been printed and just before readline starts reading input
characters. This function only exists if Python was compiled for a
...
up to be used by `rlcompleter` to complete
Python identifiers for the interactive interpreter. If the
`!readline` module is to be used with a
...
is called as `function(text, state)`, for *state* in `0`, `1`, `2`,
\..., until it returns a non-string value. It should return the next
possible completion starting with *text*.

The installed completer function is invoked by the *entry_func* callback
passed to `!rl_completion_matches` in
the underlying library. The *text* string comes from the first parameter
...
`!rl_completion_type` variable in the
underlying library as an integer.
:::
...
callback of the underlying library. The values may be different in the
same input editing scenario based on the underlying C readline
implementation. Ex: libedit is known to behave differently than
libreadline.
:::

::: function
set_completer_delims(string) get_completer_delims()
...
`!readline` module\'s history reading and
writing functions to automatically load and save a history file named
`.python_history` from the user\'s home
directory. The code below would normally be executed automatically
during interactive sessions from the user\'s
`PYTHONSTARTUP` file. :
...
`interactive mode <tut-interactive>` (see
`rlcompleter-config`).
...
The new `REPL` introduced in version 3.13
doesn\'t support readline. However, readline can still be used by
setting the `PYTHON_BASIC_REPL`
