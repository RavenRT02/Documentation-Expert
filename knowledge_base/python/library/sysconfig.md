...
::: {.module synopsis="Python's configuration information"}
sysconfig
:::
...
The `!sysconfig` module provides access to
Python\'s configuration information like the list of installation paths
and the configuration variables relevant for the current platform.
...
`!sysconfig` puts all variables found in
these files in a dictionary that can be accessed using
`get_config_vars` or
`get_config_var`.
...
`!sysconfig` under unique identifiers
based on the value returned by `os.name`. The schemes are used by package installers to determine
where to copy files to.

Python currently supports nine schemes:

- *posix_prefix*: scheme for POSIX platforms like Linux or macOS. This
...
- *posix_home*: scheme for POSIX platforms, when the *home* option is
  used. This scheme defines paths located under a specific home prefix.
...
- *posix_venv*: scheme for
  `Python virtual environments <venv>` on
  POSIX platforms; by default it is the same as *posix_prefix*.
- *nt*: scheme for Windows. This is the default scheme used when Python
  or a component is installed.
...
## User scheme {#sysconfig-user-scheme}
...
`site.USER_BASE` (written as
`{userbase}` hereafter). This scheme
installs pure Python modules and extension modules in the same location
(also known as `site.USER_SITE`).

### `posix_user`

  Path           Installation directory
  -------------- ---------------------------------------------------------------------------
  *stdlib*       `/lib/python{X.Y}`{.interpreted-text role="file"}
  *platstdlib*   `/lib/python{X.Y}`{.interpreted-text role="file"}
  *platlib*      `/lib/python{X.Y}/site-packages`{.interpreted-text role="file"}
  *purelib*      `/lib/python{X.Y}/site-packages`{.interpreted-text role="file"}
  *include*      `/include/python{X.Y}`{.interpreted-text role="file"}
  *scripts*      `/bin`{.interpreted-text role="file"}
  *data*         ``{.interpreted-text role="file"}

### `nt_user`

  Path           Installation directory
  -------------- ------------------------------------------------------------------------
  *stdlib*       `\\Python{XY}`{.interpreted-text role="file"}
  *platstdlib*   `\\Python{XY}`{.interpreted-text role="file"}
  *platlib*      `\\Python{XY}\\site-packages`{.interpreted-text role="file"}
  *purelib*      `\\Python{XY}\\site-packages`{.interpreted-text role="file"}
  *include*      `\\Python{XY}\\Include`{.interpreted-text role="file"}
  *scripts*      `\\Python{XY}\\Scripts`{.interpreted-text role="file"}
  *data*         ``{.interpreted-text role="file"}
...
### `osx_framework_user`
  ...
  Path           Installation directory
  -------------- ----------------------------------------------------------------------
  *stdlib*       `/lib/python`{.interpreted-text role="file"}
  *platstdlib*   `/lib/python`{.interpreted-text role="file"}
  *platlib*      `/lib/python/site-packages`{.interpreted-text role="file"}
  *purelib*      `/lib/python/site-packages`{.interpreted-text role="file"}
  *include*      `/include/python{X.Y}`{.interpreted-text role="file"}
  *scripts*      `/bin`{.interpreted-text role="file"}
  *data*         ``{.interpreted-text role="file"}

## Home scheme {#sysconfig-home-scheme}
...
`/usr/` or
`/usr/local/`. This scheme can be used by
anyone, regardless of the operating system they are installing for.
...
### `posix_home`
  ...
  *stdlib*        `/lib/python`{.interpreted-text role="file"}
  *platstdlib*    `/lib/python`{.interpreted-text role="file"}
  *platlib*       `/lib/python`{.interpreted-text role="file"}
  *purelib*       `/lib/python`{.interpreted-text role="file"}
  *include*       `/include/python`{.interpreted-text role="file"}
  *platinclude*   `/include/python`{.interpreted-text role="file"}
  *scripts*       `/bin`{.interpreted-text role="file"}
  *data*          ``{.interpreted-text role="file"}

## Prefix scheme {#sysconfig-prefix-scheme}
...
`/usr`, rather than the more traditional
`/usr/local`. This is entirely
...
### `posix_prefix`
  ...
  *purelib*       `/lib/python{X.Y}/site-packages`{.interpreted-text role="file"}
  *include*       `/include/python{X.Y}`{.interpreted-text role="file"}
  *platinclude*   `/include/python{X.Y}`{.interpreted-text role="file"}
  *scripts*       `/bin`{.interpreted-text role="file"}
  *data*          ``{.interpreted-text role="file"}
...
### `nt`
  ...
  Path            Installation directory
  --------------- ---------------------------------------------------------------
  *stdlib*        `\\Lib`{.interpreted-text role="file"}
  *platstdlib*    `\\Lib`{.interpreted-text role="file"}
  *platlib*       `\\Lib\\site-packages`{.interpreted-text role="file"}
  *purelib*       `\\Lib\\site-packages`{.interpreted-text role="file"}
  *include*       `\\Include`{.interpreted-text role="file"}
  *platinclude*   `\\Include`{.interpreted-text role="file"}
  *scripts*       `\\Scripts`{.interpreted-text role="file"}
  *data*          ``{.interpreted-text role="file"}

## Installation path functions

`!sysconfig` provides some functions to
determine these installation paths.
...
::: versionadded
3.10 This function was previously named `_get_default_scheme()` and
considered an implementation detail.
:::

::: versionchanged
3.11 When Python runs from a virtual environment, the *venv* scheme is
returned.
:::
:::::
...
`!sysconfig` functions that take a
*scheme* argument, such as `get_paths`.
...
::: versionchanged
3.11 When Python runs from a virtual environment and `key="prefix"`, the
*venv* scheme is returned.
:::
:::::

:::: function
[get_preferred_schemes]{#get_preferred_schemes}()
...
to the `_INSTALL_SCHEMES` module-level global value, and modify this
function to return those scheme names, to e.g. provide different schemes
for system and language package managers to use, so packages installed
by either do not mix with those by the other.

End users should not use this function, but
`get_default_scheme` and
...
`!sysconfig` stores installation paths
corresponding to each path name, for each platform, with variables to be
expanded. For instance the *stdlib* path for the *nt* scheme is:
`{base}/Lib`.

`get_path` will use the variables
returned by `get_config_vars` to expand
the path. All variables have default values for each platform so one may
call this function and get the default value.

If *scheme* is provided, it must be a value from the list returned by
`get_scheme_names`. Otherwise, the
default scheme for the current platform is used.

If *vars* is provided, it must be a dictionary of variables that will
update the dictionary returned by `get_config_vars`.
...
Return a dictionary containing all installation paths corresponding to
an installation scheme. See `get_path`
...
## Command-line usage[]{#sysconfig-cli} {#using-sysconfig-as-a-script}

You can use `!sysconfig` as a script with
Python\'s *-m* option:
