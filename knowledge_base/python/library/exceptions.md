# Built-in Exceptions {#bltin-exceptions}

In Python, all exceptions must be instances of a class that derives from
`BaseException`. In a
`try` statement with an
`except` clause that mentions a
...
Tutorial under `tut-userexceptions`.
...
handled, the new exception\'s `!__context__` attribute is automatically set to the handled exception. An
exception may be handled when an `except` or `finally` clause,
or a `with` statement, is used.

This implicit exception context can be supplemented with an explicit
cause by using `!from` with
`raise`:

    raise new_exc from original_exc

The expression following `from<raise>`
must be an exception or `None`. It will be set as
`!__cause__` on the raised exception.
Setting `!__cause__` also implicitly sets
the `!__suppress_context__` attribute to
...
`Objects/exceptions.c`. Some have
...
`Exception`). If `str` is called on an instance of this class, the representation
...
exceptions (like `OSError`) expect a
...
chaining features of `3134` became
...
associated with this exception. See also: `raise`.
:::
...
`TypeError` is raised if `note` is not a
string.
...
`add_note`. This attribute is created
...
used on a mapping or sequence is invalid: `IndexError`, `KeyError`. This can be
raised directly by `codecs.lookup`.
:::
...
`attribute-references`) or assignment
...
Raised when the `input` function hits an
end-of-file condition (EOF) without reading any data. (Note: the
`io.TextIOBase.read` and
`io.IOBase.readline` methods return an
...
Raised when a `generator` or
`coroutine` is closed; see
`generator.close` and
`coroutine.close`. It directly inherits
from `BaseException` instead of
`Exception` since it is technically not an
error.
:::
...
Raised when the `import` statement has
...
A subclass of `ImportError` which is
raised by `import` when a module could
...
A subclass of `ImportError` which is
raised when a lazy import fails because it (directly or indirectly)
...
`Control-C` or `Delete`). During execution, a check for interrupts is made
regularly. The exception inherits from `BaseException` so as to not be accidentally caught by code that catches
`Exception` and thus prevent the
...
`KeyboardInterrupt` to end the program as
...
`malloc` function), the interpreter may
...
described below. The attributes default to `None` if not specified. For backwards compatibility, if three
arguments are passed, the `~BaseException.args` attribute contains only a 2-tuple of the first two
constructor arguments.

The constructor often actually returns a subclass of
`OSError`, as described in OS
exceptions below. The particular subclass depends on
the final `.errno` value. This behaviour
only occurs when constructing `OSError`
...
is formatted by the C functions `!perror` under POSIX, and `!FormatMessage` under Windows.
:::
...
`open` or `os.unlink`), `filename` is the file
name passed to the function. For functions that involve two file system
paths (such as `os.rename`),
`filename2` corresponds to the second
...
`socket.error`,
`select.error` and
`!mmap.error` have been merged into
`OSError`, and the constructor may return
a subclass.
:::

::: versionchanged
3.4 The `filename` attribute is now the
...
This exception is derived from `RuntimeError`. It is raised when an operation is blocked during
...
`PythonFinalizationError` during the
Python finalization:

- Creating a new Python thread.
- `Joining <threading.Thread.join>` a
  running daemon thread.
- `os.fork`,
- acquiring a lock such as `threading.Lock`, when it is known that the operation would otherwise
  deadlock.

See also the `sys.is_finalizing`
function.

::: versionadded
3.13 Previously, a plain `RuntimeError`
was raised.
:::
...
`threading.Thread.join` can now raise
this exception.
:::
...
This exception may be raised when acquiring
`threading.Lock` or
`threading.RLock`.
:::
::::::
...
::: versionadded
3.5 Previously, a plain `RuntimeError` was
raised.
:::
::::
...
`weakref.proxy` function, is used to
...
Raised by built-in function `next` and an
`iterator`\'s
`~iterator.__next__` method to signal
...
When a `generator` or
`coroutine` function returns, a new
`StopIteration` instance is raised, and
the value returned by the function is used as the
`value` parameter to the constructor of
the exception.

If a generator code directly or indirectly raises
`StopIteration`, it is converted into a
`RuntimeError` (retaining the
`StopIteration` as the new exception\'s
cause).
...
`from __future__ import generator_stop`, see `479`.
:::

::: versionchanged
3.7 Enable `479` for all code by default:
a `StopIteration` error raised in a
generator is transformed into a `RuntimeError`.
:::
:::::::
...
Must be raised by `~object.__anext__`
...
`import` statement, in a call to the
...
::: versionchanged
3.10 Added the `end_lineno` and
`end_offset` attributes.
:::
::::::::::
...
`CPython`, this could be raised by
...
`Exception` so that it is not accidentally
...
implementation, `NotImplementedError` is
the proper exception to raise.

Passing arguments of the wrong type (e.g. passing a
`list` when an `int` is expected) should result in a
`TypeError`, but passing arguments with
...
`UnicodeError` has attributes that
...
non-blocking operation. Corresponds to `errno` `~errno.EAGAIN`,
`~errno.EALREADY`,
`~errno.EWOULDBLOCK` and
`~errno.EINPROGRESS`.

In addition to those of `OSError`,
`BlockingIOError` can have one more
attribute:
...
I/O classes from the `io` module.
:::
::::
...
A subclass of `ConnectionError`, raised
...
`475` for the rationale), instead of
...
`os.listdir`) is requested on something
...
it were a directory. Corresponds to `errno` `~errno.ENOTDIR`.
:::
...
::: versionchanged
3.11.1 WASI\'s `~errno.ENOTCAPABLE`
is now mapped to `PermissionError`.
:::
::::
...
::: versionadded
3.3 All the above `OSError` subclasses
were added.
:::

::: seealso
`3151` - Reworking the OS and IO exception
hierarchy
:::
...
`warning-categories` documentation for
more details.
...
(`565`). Enabling the
`Python Development Mode <devmode>` shows
this warning.

The deprecation policy is described in `387`.
:::
...
handled with `except` like all other
exceptions. In addition, they are recognised by
`except*<except_star>`, which matches
...
two classes is that `BaseExceptionGroup`
extends `BaseException` and it can wrap
any exception, while `ExceptionGroup`
extends `Exception` and it can only wrap
subclasses of `Exception`. This design is
so that `except Exception` catches an `ExceptionGroup` but not `BaseExceptionGroup`.

The `BaseExceptionGroup` constructor
returns an `ExceptionGroup` rather than a
`BaseExceptionGroup` if all contained
exceptions are `Exception` instances, so
it can be used to make the selection automatic. The
`ExceptionGroup` constructor, on the other
hand, raises a `TypeError` if any
contained exception is not an `Exception`
subclass.

Exception groups are `generic <generics>`
over the type of their contained exceptions.
...
result, as are the values of its `message`, `~BaseException.__traceback__`, `~BaseException.__cause__`,
`~BaseException.__context__` and
`~BaseException.__notes__` fields. Empty
...
Like `subgroup`, but returns the pair
...
This method is used by `subgroup` and
`split`, which are used in various
contexts to break up an exception group. A subclass needs to override it
in order to make `subgroup` and
`split` return instances of the subclass
rather than `ExceptionGroup`.

`subgroup` and `split` copy the `~BaseException.__traceback__`, `~BaseException.__cause__`,
`~BaseException.__context__` and
`~BaseException.__notes__` fields from
the original exception group to the one returned by
`derive`, so these fields do not need to
...
Note that `BaseExceptionGroup` defines
`~object.__new__`, so subclasses that
need a different constructor signature need to override that rather than
`~object.__init__`. For example, the
...
Like `ExceptionGroup`, any subclass of
`BaseExceptionGroup` which is also a
subclass of `Exception` can only wrap
