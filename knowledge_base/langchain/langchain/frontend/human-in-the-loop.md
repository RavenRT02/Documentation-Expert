...
3. The agent receives the `HITLResponse` and continues execution. Each entry in
   `decisions` may be one of:
    - ``: The agent continues executing the action
    - ``: The tool is not executed, and the agent receives the rejection message before deciding its next step
    - ``: The agent runs the tool with edited arguments
