...
If you use `InjectedState`, `InjectedStore`, `get_runtime()`, or `InjectedToolCallId`, see Migrate from older injection patterns.
:::
...
3. **Handle** the interrupt payload when the tool is invoked. Instead of running locally, the graph pauses with a payload shaped like `}`.
