...
# Nullable Boolean data type {#boolean}

## Indexing with NA values {#boolean.indexing}
...
s = pd.Series(\[1, 2, 3\]) mask = pd.array(\[True, False, pd.NA\],
dtype=\"boolean\") s\[mask\]
:::
...
`df['new_col'] = pd.Series(pd.NA, dtype="boolean")` (or another `dtype`
that supports `NA`).
...
## Kleene logical operations {#boolean.kleene}

`arrays.BooleanArray` implements [Kleene
Logic](https://en.wikipedia.org/wiki/Three-valued_logic#Kleene_and_Priest_logics)
(sometimes called three-value logic) for logical operations like `&`
...
When an `NA` is present in an operation, the output value is `NA` only
if the result cannot be determined solely based on the other input. For
example, `True | NA` is `True`, because both `True | True` and
`True | False` are `True`. In that case, we don\'t actually need to
consider the value of the `NA`.

On the other hand, `True & NA` is `NA`. The result depends on whether
the `NA` really is `True` or `False`, since `True & True` is `True`, but
