...
2.  `DataFrame`: a two-dimensional data
...
Creating a `Series` by passing a list of
values, letting pandas create a default `RangeIndex`.
...
Creating a `DataFrame` by passing a
NumPy array with a datetime index using `date_range` and labeled columns:

::: ipython
python

dates = pd.date_range(\"20130101\", periods=6) dates df =
...
:
  ...
  : \"A\": 1.0, \"B\": pd.Timestamp(\"20130102\"), \"C\": pd.Series(1,
    index=list(range(4)), dtype=\"float32\"), \"D\": np.array(\[3\] \*
    4, dtype=\"int32\"), \"E\": pd.Categorical(\[\"test\", \"train\",
    \"test\", \"train\"\]), \"F\": \"foo\",
...
have different `dtypes <basics.dtypes>`:
...
df2.abs df2.boxplot df2.add df2.C df2.add_prefix df2.clip df2.add_suffix
df2.columns df2.align df2.copy df2.all df2.count df2.any df2.combine
df2.apply df2.D df2.B df2.describe df2.diff df2.duplicated
:::
...
See the
`Essential basic functionality section <basics>`.

Use `DataFrame.head` and
`DataFrame.tail` to view the top and
...
Display the `DataFrame.index` or
`DataFrame.columns`:
...
`DataFrame.to_numpy` without the index or
column labels:
...
`DataFrame.to_numpy`, pandas will find
the NumPy dtype that can hold *all* of the dtypes in the DataFrame. If
the common data type is `object`, `DataFrame.to_numpy` will require copying data.

::: ipython
python

df2.dtypes df2.to_numpy()
:::
:::::

`~DataFrame.describe` shows a quick
...
and `MultiIndex / Advanced Indexing <advanced>`.
...
See more in `Selection by Label <indexing.label>` using `DataFrame.loc` or
`DataFrame.at`.
...
See more in `Selection by Position <indexing.integer>` using `DataFrame.iloc` or
`DataFrame.iat`.
...
: \[1, 2, 3, 4, 5, 6\], index=pd.date_range(\"20130102\", periods=6))
...
`DataFrame.dropna` drops any rows that
have missing data:
...
`isna` gets the boolean mask where values
are `nan`:
...
Operating with another `Series` or
`DataFrame` with a different index or
...
`DataFrame.agg` and
`DataFrame.transform` applies a user
...
`Series` is equipped with a set of
...
the `.DataFrameGroupBy.sum` function to
...
The unique values of the grouped column now form the index of the
result.
...
See the sections on
`Hierarchical Indexing <advanced.hierarchical>` and `Reshaping <reshaping.stacking>`.
...
:
  ...
  : \"A\": \[\"one\", \"one\", \"two\", \"three\"\] \* 3, \"B\":
    \[\"A\", \"B\", \"C\"\] \* 4, \"C\": \[\"foo\", \"foo\", \"foo\",
    \"bar\", \"bar\", \"bar\"\] \* 2, \"D\": np.random.randn(12), \"E\":
    np.random.randn(12),
...
`DataFrame` specifying the `values`,
`index` and `columns`
...
rng = pd.date_range(\"1/1/2012\", periods=100, freq=\"s\") ts =
pd.Series(np.random.randint(0, 500, len(rng)), index=rng)
ts.resample(\"5Min\").sum()
:::

`Series.tz_localize` localizes a time
series to a time zone:

::: ipython
python

rng = pd.date_range(\"3/6/2012 00:00\", periods=5, freq=\"D\") ts =
pd.Series(np.random.randn(len(rng)), rng) ts ts_utc =
ts.tz_localize(\"UTC\") ts_utc
:::

`Series.tz_convert` converts a timezones
...
`categorical introduction <categorical>`
and the `API documentation <api.arrays.categorical>`.
...
: {\"id\": \[1, 2, 3, 4, 5, 6\], \"raw_grade\": \[\"a\", \"b\", \"b\",
  \"a\", \"a\", \"e\"\]}
...
(methods under `Series.cat` return a new
`Series` by default):
...
plt.figure(); df.plot(); \@savefig frame_plot_basic.png
plt.legend(loc=\'best\');
:::
...
`Series` or
`DataFrame` you might see an exception
like:
...
See `Comparisons<basics.compare>` and
`Gotchas<gotchas>` for an explanation and
what to do.