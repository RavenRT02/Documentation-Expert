...
# Time deltas {#timedeltas.timedeltas}
...
including [ISO 8601
Duration](https://en.wikipedia.org/wiki/ISO_8601#Durations) strings.
...
\# strings pd.Timedelta(\"1 days\") pd.Timedelta(\"1 days 00:00:00\")
pd.Timedelta(\"1 days 2 hours\") pd.Timedelta(\"-1 days 2 min 3us\")

\# like datetime.timedelta \# note: these MUST be specified as keyword
arguments pd.Timedelta(days=1, seconds=1)
...
\# negative Timedeltas have this string repr \# to be more consistent
with datetime.timedelta conventions pd.Timedelta(\"-1us\")

\# a NaT pd.Timedelta(\"nan\") pd.Timedelta(\"nat\")

\# ISO 8601 Duration strings pd.Timedelta(\"P0DT0H1M0S\")
pd.Timedelta(\"P0DT0H0M0.000000123S\")
:::

`DateOffsets<timeseries.offsets>`
(`Hour, Minute, Second, Milli, Micro, Nano`) can also be used in
construction.
...
The `unit` keyword argument specifies the unit of the Timedelta if the
input is numeric:

::: ipython
python

pd.to_timedelta(np.arange(5), unit=\"s\") pd.to_timedelta(np.arange(5),
unit=\"D\")
:::
...
If a string or array of strings is passed as an input then the `unit`
keyword argument will be ignored. If a string without units is passed
then the default unit of nanoseconds is assumed.
::::

### Timedelta limitations {#timedeltas.limitations}

`Timedelta` uses a 64-bit integer to
represent time, so the representable range depends on the chosen
resolution (`unit`). The class-level attributes
`Timedelta.min`,
`Timedelta.max`, and
`Timedelta.resolution` default to
nanosecond limits:

::: ipython
python

pd.Timedelta.min pd.Timedelta.max pd.Timedelta.resolution
:::

On a `Timedelta` *instance*, the same
...
td = pd.Timedelta(1, unit=\"s\") td.min td.max td.resolution
:::
...
s = pd.Series(pd.date_range(\"2012-1-1\", periods=3, freq=\"D\")) td =
pd.Series(\[pd.Timedelta(days=i) for i in range(3)\]) df =
pd.DataFrame({\"A\": s, \"B\": td}) df df\[\"C\"\] = df\[\"A\"\] +
df\[\"B\"\] df df.dtypes

s - s.max() s - datetime.datetime(2011, 1, 1, 3, 5) s +
datetime.timedelta(minutes=5) s + pd.offsets.Minute(5) s +
pd.offsets.Minute(5) + pd.offsets.Milli(5)
:::
...
s.max() - s datetime.datetime(2011, 1, 1, 3, 5) - s
...
A = s - pd.Timestamp(\"20120101\") - pd.Timedelta(\"00:05:05\") B = s -
pd.Series(pd.date_range(\"2012-1-2\", periods=3, freq=\"D\"))
...
You can fillna on timedeltas, passing a timedelta to get a particular
value.

::: ipython
python

y.fillna(pd.Timedelta(0)) y.fillna(pd.Timedelta(10, unit=\"s\"))
...
td1 = pd.Timedelta(\"-1 days 2 hours 3 seconds\") td1 -1 \* td1 -td1
abs(td1)
:::

## Reductions {#timedeltas.timedeltas_reductions}
...
: pd.to_timedelta(\[\"-1 days +00:00:05\", \"nat\", \"-1 days
  +00:00:05\", \"1 days\"\])

) y2 y2.mean() y2.median() y2.quantile(0.1) y2.sum()
:::

## Frequency conversion {#timedeltas.timedeltas_convert}

Timedelta Series and `TimedeltaIndex`, and `Timedelta` can be converted
to other frequencies by astyping to a specific timedelta dtype.

::: ipython
python

december = pd.Series(pd.date_range(\"20121201\", periods=4)) january =
pd.Series(pd.date_range(\"20130101\", periods=4)) td = january -
december
...
For timedelta64 resolutions other than the supported \"s\", \"ms\",
\"us\", \"ns\", an alternative is to divide by another timedelta object.
Note that division by the NumPy scalar is true division, while astyping
...
These are identical to the values returned by `datetime.timedelta`, in
that, for example, the `.seconds` attribute represents the number of
seconds \>= 0 and \< 1 day. These are signed according to whether the
`Timedelta` is signed.

These operations can also be directly accessed via the `.dt` property of
the `Series` as well.
...
tds = pd.Timedelta(\"31 days 5 min 3 sec\") tds.days tds.seconds
(-tds).seconds
:::

You can use the `.components` property to access a reduced form of the
timedelta. This returns a `DataFrame` indexed similarly to the `Series`.
...
::: {#timedeltas.isoformat}
You can convert a `Timedelta` to an [ISO 8601
Duration](https://en.wikipedia.org/wiki/ISO_8601#Durations) string with
...
## TimedeltaIndex {#timedeltas.index}

To generate an index with time delta, you can use either the
`TimedeltaIndex` or the
`timedelta_range` constructor.

Using `TimedeltaIndex` you can pass string-like, `Timedelta`,
`timedelta`, or `np.timedelta64` objects. Passing `np.nan/pd.NaT/nat`
...
:
  ...
  : \"1 days\", \"1 days, 00:00:05\", np.timedelta64(2, \"D\"),
...
pd.TimedeltaIndex(\[\"0 days\", \"10 days\", \"20 days\"\],
freq=\"infer\")
:::

### Generating ranges of time deltas

Similar to `date_range`, you can
construct regular ranges of a `TimedeltaIndex` using
`timedelta_range`. The default frequency
...
The `freq` parameter can passed a variety of
...
spaced timedeltas from `start` to `end` inclusively, with `periods`
number of elements in the resulting `TimedeltaIndex`:
...
: np.arange(100), index=pd.timedelta_range(\"1 days\", periods=100,
  freq=\"h\"),
...
s\[\"1 day\":\"2 day\"\] s\[\"1 day 01:00:00\"\] s\[pd.Timedelta(\"1 day
1h\")\]
:::
...
tdi.to_list() dti = pd.date_range(\"20130101\", periods=3) dti.to_list()
(dti + tdi).to_list() (dti - tdi).to_list()
:::
...
\# adding or timedelta and date -\> datelike tdi +
pd.Timestamp(\"20130101\")

\# subtraction of a date and a timedelta -\> datelike \# note that
trying to subtract a date from a Timedelta will raise an exception
(pd.Timestamp(\"20130101\") - tdi).to_list()

\# timedelta + timedelta -\> timedelta tdi + pd.Timedelta(\"10 days\")

\# division can result in a Timedelta if the divisor is an integer tdi /
2
...
Similar to
`timeseries resampling <timeseries.resampling>`, we can resample with a `TimedeltaIndex`.
